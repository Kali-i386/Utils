...
def execute():
    ...
    def reg():
        global total,execstring,re_match
        re_match = pattern_entry.get()
        total = 0
        execstring = f"global regexobj;regexobj = re.compile('{re_match}'"
        for i in ["I", "M", "S", "U", "X"]:
            exec(f"""
global execstring
if {i} == 1: 
        ...
        try:
            try:
                for e in range(32767):
                    print("Result", result.group(e))
                    match.insert('end', result.group(e))
            except IndexError:
                pass
            except re.error:
                pass
